...
export default function Home() {
  ...
  const handleSubmit = async e =>{
   ...
   const res = await fetch(`http://${process.env.BACKENDHOST}:5000/predict`,{
        method: "POST",
        body: JSON.stringify({
          text: toxicity_text.current.value}),
        headers:{
  ...
  return (
    ...
    <div className={styles.wrap}>
      <div className={styles.containerTitle}>
          <h1 className={styles.title}>Toxicity</h1>
      </div>
      <div className={styles.search}>
        <form onSubmit={handleSubmit}>
          
            <input ref={toxicity_text} type="text" className={styles.searchTerm} placeholder="What are you looking for?"/>
